...
void UStateManagerComponent::SwitchStateByKey(const FString Key)
{
	if(!bInitialized)
	{
		PrintDebugMessage(FString::Printf(TEXT(" State Manager not initialized!")));
...
void UStateManagerComponent::AddState(FString StateKey, UStateBase* State)
{
	// Add the state to the map
	State->SetStateManager(this);
	State->StateDisplayName = FName(StateKey);
	StateMap.Add(StateKey, State);
}
